...
> 基于 Electron 的桌面应用，从抖音视频采集、转录、AI 洗稿到 Skills 蒸馏、视频生成与多平台发布
...
成片之后进入**发布中心**按渠道交付：**抖音图文**与**今日头条文章**已由本机浏览器自动提交（提交前必经预览；提交后只记「已提交」，是否真的发出由人工到平台后台核实后点「标记已发布」），**视频与其余平台仍以人工交付为主**。界面按渠道分栏（抖音图文 / 今日头条文章 / 视频人工交付），两类自动通路的引擎都是本机跑、零云端依赖。
